...
# dplyr套件的主要應用
# filter() ：挑選符合條件資料，列挑選，可設：聯集&，交集：|
# select()：篩選變數，欄挑選
# mutate() ：新增變數，可計算後新增變數，如：mutate(某資料, 某欄位 = 某欄位 / 10000)
# arrange()：排序-由小到大，如果要由大到小--desc()
# group_by ：資料分類別處理
# summarise ：快速找出變數的資料特徵值
...
library(gapminder)
# colname(): 瞭解重要資料欄位：country （國家）、 continent （洲）、year （年）
#lifeExp(預期平均壽命）、pop（人口數）、“gdpPercap (人均 GDP)”
# structure()：瞭解各欄位資料型態
# unique(gapminder$country) #查看國家欄位有哪些獨立資料值
# filter 使用
...
# group_by()
# gapminder %>% group_by(year) %>%summarise(sum(as.numeric(pop)))#以年度彙總，算出pop會總值
...
names(gap_year) <- c(" year ", " sum_pop ") #更改欄位名稱
gap_year#顯示結果
...
sales %>% group_by(area) %>%
  summarise(sum(as.numeric(profit2)))
boxplot(profit2~area, data = sales)
# 事實上關係還是看的不很清楚
...
#，可以對你的經營決策有幫助---可能成為期中作業
#練習21--北部地區哪幾個城市利潤最高
#練習22--北部地區-豪華型，哪個城市利潤最高
#練習23--北部地區-哪種類別產品利潤最高
